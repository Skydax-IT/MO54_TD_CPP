...
MyIterator data_list::begin() {
    return MyIterator(*this);
}

MyIterator data_list::end() {
    MyIterator end_iter=MyIterator(*this);
    end_iter.loc=this->_values.size()-1;
    return end_iter;
}

MyIterator::MyIterator(data_list& datalist) : dataList {datalist}, loc {0} {};

double MyIterator::get_value() {
    return this->dataList._values.get_value(this->loc);
}

MyIterator &MyIterator::operator++() {
    if(this->loc==this->dataList._values.size()-1){
...
double MyIterator::operator[](int i) {
    if( i<0 || i>=this->dataList._values.size() ){
        cerr<<"out of range!"<<endl;
        throw;
    }
    loc=i;
